...
Auth uses Supabase Auth with Google OAuth. Application roles live in Supabase Auth `app_metadata.roles`; do not use user-editable metadata for authorization. Booking, ticket, and counter data lives in `grandfeasteu_*` Supabase tables scoped by `APP_EVENT_ID`; production uses `77 Labs Prod`, live development uses `77 Labs Test`, and local development uses the Supabase CLI local database.
