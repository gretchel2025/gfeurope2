...
`booking.tickets_email_sent`, `ticket.created`, `ticket.checked_in`,
`ticket.checked_out`, and `ticket_counter.available_added`. Actor types are `public`,
`admin`, and `system`. Entity types are `booking`, `ticket`, and `ticket_counter`.
Audit metadata may include operational identifiers and state changes, but must not
include secrets, tokens, uploaded file contents, payment proof URLs, or email bodies.
