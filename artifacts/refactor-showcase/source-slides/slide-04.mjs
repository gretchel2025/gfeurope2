...
export async function slide04(presentation) {
	...
	const phasesTop = [
		{
			...
			text: 'Create the new architecture skeleton and naming conventions first.',
			fill: '#EAF2F8',
			accent: theme.blue
		},
		{
			title: 'Normalize the domain',
			text: 'Move models, enums, and business rules into clearer domain modules.',
			fill: '#F0ECF9',
			accent: '#6A5AA6'
		},
		{
			title: 'Decouple the services',
			text: 'Replace framework-aware workflows with application services and app-level errors.',
			fill: '#E4F1EA',
			accent: theme.green
		}
	];
	const phasesBottom = [
		{
			title: 'Rebuild persistence seams',
			text: 'Move Mongo concerns into repository implementations behind ports.',
			fill: '#F5EFE7',
			accent: theme.warmDark
		},
		{
			title: 'Centralize the platform',
			text: 'Gather auth, config, bootstrap, logging, media, and email under infrastructure.',
			fill: '#E6F0EE',
			accent: theme.teal
		},
		{
			title: 'Thin the routes and clean house',
			text: 'Convert routes into adapters, remove noisy leftovers, and smoke test the main flows.',
			fill: '#FBEEE4',
			accent: theme.warm
		}
	];
				...
				children: [
					...
					jsx(MetricStrip, {
						items: [
							...
							{
								value: '1',
								label: 'core principle: add the new path before deleting the old path'
							},
